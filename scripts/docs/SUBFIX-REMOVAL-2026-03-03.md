...
- 運用方針を更新し、画像パスの親子コンパイル互換性確保のため `\subfix{}` を再適用。
- 2026-03-10 時点で `\includegraphics` の `\subfix{}` 未適用箇所は 0 件を確認。
- `hsm_tok_q.tex` 直接コンパイル時の停止要因は、`2008/1st/01/08` 配下の既存欠落画像であり、
  `\subfix{}` 再適用差分とは別件。
